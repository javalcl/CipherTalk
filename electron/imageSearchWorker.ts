...
function walkForDat(
  ...
  // 根据模式选择：
  // - thumbOnly: 只要缩略图
  // - allowThumbnail: 优先缩略图（加载快），没有才用高清图
  // - !allowThumbnail: 只要高清图
  let finalPool: Candidate[]
  if (thumbOnly) {
    // 只要缩略图
    finalPool = basePool.filter((item) => item.isThumb)
    if (!finalPool.length) finalPool = basePool
  } else if (allowThumbnail) {
    // 优先缩略图
    const thumbs = basePool.filter((item) => item.isThumb)
    finalPool = thumbs.length ? thumbs : basePool
  } else {
    // 只要高清图
    const nonThumb = basePool.filter((item) => !item.isThumb)
    finalPool = nonThumb.length ? nonThumb : basePool
  }
